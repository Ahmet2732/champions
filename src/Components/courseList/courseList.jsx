  ...
  const handleSearch = async (e) => {
    ...
    if (query) {
      try {
        ...
        setFilteredCourses(response.data.data.data); // Update filtered courses
      } catch (err) {
        console.error('Error searching courses:', err);
        setError('Failed to search courses');
      }
    } else {
      setFilteredCourses(courses); // IF search is clear
    }
  };
                ...
                currentCourses.map((course) => (
                  <tr key={course.id}>
                    ...
                    <td>
                      <span
                        className={`badge ${
                          course.status === 'Active' ? 'bg-danger' : 'bg-success'
                        }`}
                      >
                        {course.status}
                      </span>
